  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-900 leading-tight">AI Plan Review</h2>
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mt-1">Review, tweak, and approve AI generated meal plans before athletes see them.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-4">
          <Card className="rounded-2xl shadow-sm border border-slate-200">
            <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-4">
              <CardTitle className="text-sm font-bold uppercase text-slate-500 tracking-wider">Team Athletes</CardTitle>
            </CardHeader>
            <CardContent className="p-4 overflow-y-auto max-h-[500px]">
              <ul className="space-y-2">
                {athletes.length === 0 ? (
                  <p className="text-center text-sm text-slate-500 py-4">No athletes found in your team.</p>
                ) : (
                  athletes.map(athlete => (
                    <ReviewItem 
                      key={athlete.id}
                      name={athlete.displayName || athlete.email} 
                      context={athlete.goals || "Athlete"} 
          ...
          {!selectedAthlete ? (
            <Card className="rounded-2xl shadow-sm border border-slate-200 h-full flex items-center justify-center min-h-[400px]">
              <div className="text-center text-slate-500">
                ...
                <p>Select an athlete to view or generate their meal plan.</p>
              </div>
            </Card>
          ) : (
            <Card className="rounded-2xl shadow-sm border border-slate-200 h-full flex flex-col overflow-hidden">
              <div className="bg-[#064E3B] text-white p-4 flex justify-between items-center">
                <div>
                  <h3 className="font-bold text-lg leading-tight">Reviewing: {selectedAthlete.displayName || selectedAthlete.email}</h3>
                  <p className="text-emerald-300 text-[10px] font-bold uppercase tracking-widest mt-1 flex gap-3">
                    <span>Age: {selectedAthlete.age || '?'}</span> <span>Weight: {selectedAthlete.weight || '?'}kg</span>
                  </p>
                </div>
                {mealPlan?.status && (
              ...
              <div className="flex-1 p-6 flex flex-col gap-4 bg-slate-50 min-h-[500px]">
                {!editorContent ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-slate-500 border-2 border-dashed border-slate-200 rounded-xl p-8">
                    <Bot className="h-12 w-12 mb-3 text-slate-300" />
                    <p className="mb-4 text-center">No plan exists for tomorrow.<br/>Generate one using AI based on their upcoming training.</p>
                  ...
                  <>
                    <div className="bg-white border-2 border-amber-400 p-4 rounded-xl text-sm shadow-sm relative overflow-hidden">
                      <span className="text-[10px] font-bold text-amber-600 uppercase tracking-widest absolute top-2 right-3">AI Context</span>
                      <p className="text-sm font-semibold text-[#064E3B] mt-2">
                        Plan generated for tomorrow based on athlete stats and upcoming training events.
                      </p>
                    </div>
                    
                    <div className="bg-slate-900 rounded-xl p-5 flex flex-col flex-1">
                      <div className="mb-4">
                        <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Plan Editor Mode</span>
                      </div>
                      <Textarea 
                        className="flex-1 resize-none font-mono text-xs leading-relaxed p-4 bg-white/5 border-white/10 text-white/90 rounded-lg focus-visible:ring-1 focus-visible:ring-emerald-500 focus-visible:border-transparent placeholder:text-white/20 min-h-[300px]" 
                        value={editorContent}
                    ...
                    <div className="mt-4 flex gap-3">
                      <Button onClick={handleGenerateAI} disabled={isGenerating} variant="outline" className="flex-1 py-4 text-[11px] font-bold text-slate-700 hover:bg-slate-100 rounded-lg uppercase">
                        {isGenerating ? <Loader2 className="mr-2 h-3 w-3 animate-spin" /> : <Edit className="mr-2 h-3 w-3" />} Ask AI to Revise
                      </Button>
                      <Button onClick={handleApproveAndPublish} disabled={isSaving} className="flex-1 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-[11px] font-bold uppercase transition-all shadow-sm">
                        {isSaving ? <Loader2 className="mr-2 h-3 w-3 animate-spin" /> : <Check className="mr-2 h-3 w-3" />} Approve & Publish
                      </Button>
                    </div>
                  </>
                )}
...
function ReviewItem({ name, context, active = false, onClick }: any) {
  return (
    <li onClick={onClick} className={`p-4 rounded-xl cursor-pointer transition-all border ${active ? 'bg-[#064E3B] text-white border-[#064E3B] shadow-md' : 'bg-white border-slate-200 hover:bg-slate-50 text-slate-700 hover:shadow-sm'}`}>
      ...
      <div className={`text-[11px] font-medium flex items-center ${active ? 'text-emerald-300' : 'text-slate-500'}`}>
        <FileText className="inline mr-1 w-3 h-3" /> {context}
      </div>
    </li>
  );
}
