  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-900 leading-tight">Team Overview</h2>
          <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mt-1">Manage macro-cycles and set training loads.</p>
        </div>
        <div className="flex gap-3">
          {profile?.teamId && (
            <Button variant="outline" onClick={handleCopy} className="text-slate-500 border-slate-200">
              <span className="font-mono text-xs mr-2">{profile.teamId}</span>
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger render={<Button className="bg-[#F59E0B] text-[#064E3B] font-bold rounded-lg shadow-sm hover:shadow-md transition-all hover:bg-[#D97706] hover:text-white" />}>
              <CalendarIcon className="mr-2 h-4 w-4" /> Add Session
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                <DialogTitle>Add Training Session</DialogTitle>
                <DialogDescription>Create a new event for the team calendar.</DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Select value={intensity} onValueChange={setIntensity}>
                    <SelectTrigger><SelectValue placeholder="Intensity Level" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="Low">Low</SelectItem>
                      <SelectItem value="Medium">Medium</SelectItem>
                      <SelectItem value="High">High</SelectItem>
                      <SelectItem value="Match">Match (Maximum)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              <DialogFooter>
                <Button onClick={handleAddSession} disabled={isSubmitting} className="bg-[#064E3B] text-white">
                  {isSubmitting ? "Adding..." : "Save Session"}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 rounded-2xl shadow-sm border border-slate-200">
          <CardHeader className="border-b border-slate-100 bg-slate-50/50">
            <CardTitle className="text-sm font-bold uppercase text-slate-500 tracking-wider">This Week's Intensity Load</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            <div className="grid grid-cols-7 gap-2">
              <DayCol day="Mon" intensity={getIntensityForDay(1)} />
              <DayCol day="Tue" intensity={getIntensityForDay(2)} />
              <DayCol day="Wed" intensity={getIntensityForDay(3)} />
              <DayCol day="Thu" intensity={getIntensityForDay(4)} />
              <DayCol day="Fri" intensity={getIntensityForDay(5)} />
              <DayCol day="Sat" intensity={getIntensityForDay(6)} />
              <DayCol day="Sun" intensity={getIntensityForDay(0)} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
          <CardHeader className="border-b border-slate-100 bg-slate-50/50 shrink-0">
            <CardTitle className="flex justify-between items-center text-sm font-bold uppercase text-slate-500 tracking-wider">
              Team Roster <Badge variant="secondary" className="bg-slate-200 text-slate-700">{teamMembers.length} Active</Badge>
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0 flex-1 overflow-y-auto min-h-[300px]">
            {teamMembers.length === 0 ? (
              <div className="p-8 text-center text-slate-500 flex flex-col items-center justify-center h-full">
                ...
                <p className="text-sm font-medium">No athletes have joined yet.</p>
                <p className="text-xs mt-1">Share your Team ID with them.</p>
              </div>
            ) : (
              <ul className="divide-y divide-slate-100">
                {teamMembers.map((member) => (
                  <PlayerRow key={member.id} name={member.displayName || member.email} status="Ready" />
                ))}
              </ul>
            )}
          </CardContent>
